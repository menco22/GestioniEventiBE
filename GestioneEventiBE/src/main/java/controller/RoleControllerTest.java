...
public class RoleControllerTest {
	...
	public void testDoGetHttpServletRequestHttpServletResponse() throws IOException, InterruptedException {
		...
		// request senza login
		HttpRequest request401 = HttpRequest.newBuilder().uri(URI.create("http://localhost:8080/GestioneEventiBE/RoleController"))
        		.GET().build();
		...
		// request per lista ruoli
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("http://localhost:8080/GestioneEventiBE/RoleController"))
        ...
        // request per ruolo specifico
        HttpRequest requestId = HttpRequest.newBuilder().uri(URI.create("http://localhost:8080/GestioneEventiBE/RoleController?id=1"))
        		.header("Cookie", loginCookie[1]).GET().build();
